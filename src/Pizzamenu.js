...
function Pizzamenu() {
  ...
  const filteredPizzas = pizzaData.filter((pizza) => {
    if (filter === "all") {
      ...
      return (
        ...
        pizza.ingredients.includes("spinach")
      );
    } else if (filter === "meaty") {
      return (
        pizza.ingredients.includes("pepperoni") ||
        pizza.ingredients.includes("ham")
      );
      ...
      return (
        !pizza.ingredients.includes("mozarella") &&
        !pizza.ingredients.includes("ham") &&
        !pizza.ingredients.includes("pepperoni") &&
        !pizza.ingredients.includes("burrata")
      );
      ...
      return (
        !pizza.ingredients.includes("mozarella") &&
        !pizza.ingredients.includes("ricotta") &&
        !pizza.ingredients.includes("burrata")
      );
  ...
  return (
    <div>
      ...
      <p className="p1">
        Authentic Italian pizza. 6 different pizzas to choose from.
      </p>
      ...
      {isOpen && (
        <div>
          {selectedPizzas.length === 0 ? (
            <p className="p2">Select a pizza to order</p>
          ) : (
            <p className="p2">
              Click on the pizza to remove it from your order
            </p>
          )}
